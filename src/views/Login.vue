                ...
                <div class="max-w-md mx-auto">
                    ...
                    <div class="divide-y divide-gray-200">
                        <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                            <div class="relative">
                                <input v-model="name" autocomplete="off" id="username" name="username" type="text" class="peer placeholder-transparent h-10 w-full border-b-2 border-gray-300 text-gray-900 focus:outline-none focus:borer-rose-600" placeholder="Email address" />
...
<script>
    $(function(){
        $(document).attr("title", "Memory | Login");
    });
export default {
    data() {
        return {
    ...
    methods: {
        submitForm(event) {
            ...
            axios.post('api', {name: this.name, password: this.password})
                .then(res => {
                    ...
                    location.href = '/';
                    setTimeout(() => {
                        localStorage.setItem('e1aa9dc695', res.data.token)
                    }, 1000);
                })
                .catch(error => {
                    // handle error
